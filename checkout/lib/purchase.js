...
export function isMacPurchase(session, prices) {
  if (!session || session.mode !== 'payment' ||
      session.metadata?.product !== PRODUCT_MARKER || prices.size === 0) return false;
...
export function checkoutProductMarker() { return PRODUCT_MARKER; }
